...
class FlashAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> \
        ...
        cu_q_lens = torch.arange(0, (bsz + 1) * q_len,
                                 ...
                                 dtype=torch.int32,
                                 device=qkv.device)
        output = flash_attn_varlen_qkvpacked_xla(qkv,
                                                 cu_q_lens,
                                                 max_s,
                                                 0.0,
                                                 softmax_scale=None,
...
def test_flash_attn_rewriter(enable_tp_attention, dtype):
    ...
    model.train()
    hidden_states = torch.linspace(-0.5,
                                   0.5,
                                   BATCH_SIZE * SEQ_LEN * DIMS * N_HEADS,
                                   device=device).reshape(
                                       BATCH_SIZE, SEQ_LEN,
                                       DIMS * N_HEADS).to(dtype)
    ...
    model_fa.train()
    hidden_states_fa = torch.linspace(-0.5,
                                      0.5,
                                      BATCH_SIZE * SEQ_LEN * DIMS * N_HEADS,
                                      device=device).reshape(
                                          BATCH_SIZE, SEQ_LEN,
    ...
    for idx, (ret, ret_fa) in enumerate(zip(rets, rets_fa)):
        allclose_fwd = torch.allclose(ret.cpu().detach(),
                                      ret_fa.cpu().detach(),
                                      rtol=1e-5,
                                      atol=1e-5)
        allclose_bwd = torch.allclose(model.q_proj.weight.cpu().detach(),
